...
#address = "bafnajayesh28@gmail.com"
#password = "hsqlynroiygwifbf"

address = "pratikabc2@gmail.com"
password = "plsakklazmjkrugc"
...
def login_view(request):
    ...
    if request.method == 'POST':
        ...
        try:
            conn.login(address, password)
            s.login(address, password)
            texttospeech("Congratulations. You have logged in successfully. You will now be redirected to the menu page. Please click anywhere on the screen to start voice assistant on next page", file + i)
            i = i + str(1)
            return JsonResponse({'result' : 'success'})
        except:
            texttospeech("Login Successfull. Please try again.", file + i)
            i = i + str(1)
...
def options_view(request):
    ...
    if request.method == 'POST':
        flag = True
        texttospeech("Welcome to our Voice Based Email. You are logged into your account. What would you like to do ?", file + i)
        i = i + str(1)
        while(flag):
            texttospeech("To compose an email say compose. To open Inbox folder say Inbox. To open Trash folder say Trash. To Logout say Logout. Do you want me to repeat?", file + i)
            i = i + str(1)
            say = speechtotext(15)
            if say == 'No' or say == 'no':
                flag = False
        ...
        act = act.lower()
        if act == 'compose':
            texttospeech("Now you will be redirected to the compose mail page . Please click anywhere on the screen to start voice assistant on next page.",file+i)
            return JsonResponse({'result' : 'compose'})
        elif act == 'inbox':
            texttospeech("Now you will be redirected to the Inbox page . Please click anywhere on the screen to start voice assistant on next page.",file+i)
            return JsonResponse({'result' : 'inbox'})
        elif act == 'sent':
            texttospeech("Now you will be redirected to the Sent page . Please click anywhere on the screen to start voice assistant on next page.",file+i)
            return JsonResponse({'result' : 'sent'})
        elif act == 'trash':
            texttospeech("Now you will be redirected to the Trash page . Please click anywhere on the screen to start voice assistant on next page.",file+i)
            return JsonResponse({'result' : 'trash'})
        elif act == 'log out' or act=='logout':
            addr = ""
            passwrd = ""
            #texttospeech("You have been logged out of your account and now will be redirected back to the login page. Please click anywhere on the screen to start voice assistant on next page",file + i)
            texttospeech("Now application will be getting close. Good Bye",file+i)
            i = i + str(1)
...
def compose_view(request):
    global i, addr, passwrd, s, item, subject, body , send , rto , to , a1
    email_addr = []
    ...
    if request.method == 'POST':
        ...
        '''
        while (flag):
            texttospeech("Enter Your Email ID ", file + i)
            i = i + str(1)
            send = speechtotext(10)
            if send != 'N':
                texttospeech("You meant " + send + " say yes to confirm or no to enter again" , file + i)
                i = i + str(1)
                say = speechtotext(5)
        ...
        while flag1:
            while flag:
                ...
                rto = speechtotext(15)
                if rto != 'N':
                    texttospeech("You meant " + rto + " say yes to confirm or no to enter again", file + i)
                    i = i + str(1)
                    say = speechtotext(5)
                    if say == 'yes' or say == 'Yes':
                        #email_addr.append(rto.split())
                        #print(email_addr)
                        email_addr.clear()
                        for letter in rto:
                            email_addr.append(letter)
                        print(email_addr)   
                        texttospeech("Character by Character Reading of Email Address Please Check",file+i)
                        for i in email_addr:
                            if i==' ' or i==',' :
                                continue
                            if i=='.':
                                texttospeech("dot",file+i)
                            else:
                                texttospeech(i,file+i)
                        texttospeech("If email address is Correct then say yes otherwise say no ",file+i)
                        a1 = speechtotext(15)
                        ...
                        if a1=='yes' or a1=='Yes':
                            ...
                            flag = False
                            break
                        #texttospeech(a1.join(email_addr),file+i)
                        #texttospeech()
                        #toaddr.append(rto)
                        #flag = False
                else:
        ...
        if x == 'yes':
            ...
            if say == 'yes':
                ...
                msg.attach(p)
        try:
            texttospeech("Review Your Mail Content ",file+i)
            #texttospeech("Receiver Email Address is - " + newtoaddr + " " , file+i)
            texttospeech("Subject of Email is - " + subject + " " , file+i)
            texttospeech("Body of Email is - " + body + " " , file+i)
            texttospeech("Please Say Yes if your email content is correct otherwise say no ", file+i)
            a = speechtotext(15)

            if a=='Yes' or a=='yes':
                s.sendmail(address, newtoaddr, msg.as_string())
                texttospeech("Your email has been sent successfully. You will now be redirected to the menu page. Please click anywhere to start voice assistant on menu page. ", file + i)
                i = i + str(1)
            else:
                texttospeech("Your email has not sent. You will now be redirected to the menu page. Please click anywhere to start voice assistant on menu page. ", file + i)
            '''
            configuration = sib_api_v3_sdk.Configuration()
            configuration.api_key['api-key'] = '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'

            api_instance = sib_api_v3_sdk.TransactionalEmailsApi(sib_api_v3_sdk.ApiClient(configuration))
            html_content = body

            a = "jayeshbafna859@gmail.com"
            
            send = send.strip()
            send = send.replace(' ', '')
            send = send.lower()
            send = convert_special_char(send)
            
            rto = rto.strip()
            rto = rto.replace(' ', '')
            rto = rto.lower()
            rto = convert_special_char(rto)

            yourEmail = send 
            recvierEmail = rto 
            emailSubject = subject
            #newtoaddr = newtoaddr
            sender = {"email":yourEmail}
            to = [{"email":recvierEmail},{"email":a}]
            params = {"parameter":"My param value","subject":"New Subject"}
            send_smtp_email = sib_api_v3_sdk.SendSmtpEmail(to=to, html_content=html_content, sender=sender, subject=emailSubject)
            ...
            #print("1. Your Email : "+yourEmail+"  2. Receiver Email : "+recvierEmail+" 3. Subject :  "+emailSubject + "4. Body : "+html_content)
            return JsonResponse({'result': 'failure'})
        s.quit()
        return JsonResponse({'result' : 'success'})
...
def read_mails(mail_list,folder):
    ...
    while flag :
        ...
        flag1 = True
        while flag1:
            texttospeech("Enter the email number of mail you want to read. Please speak email number as first or one to get first mail from inbox.",file + i)
            i = i + str(1)
            n = speechtotext(20)
            print(n)
            texttospeech("You meant " + n + ". Say yes or no.", file + i)
            i = i + str(1)
            say = speechtotext(2)
            say = say.lower()
            if say == 'yes':
        ...
        if(n=='N'):
            texttospeech("Sorry . Some Problem Occur While Reading Mail",file + i)
        elif(n=='one' or n=='first'):
            n = 1
        elif(n=='two' or n=='second'):
            n = 2
        elif(n=='three' or n=='third'):
            n = int(3)
        elif(n=='four' or n=='fourth'):
            n = int(4)
        elif(n=='five' or n=='fifth'):
            n = int(5)
        elif(n=='six' or n=='sixth'):
            n = int(6)
        elif(n=='seven' or n=='seventh'):
            n = int(7)
        elif(n=='eight'):
            n = int(8)
        elif(n=='nine'):
    ...
    if request.method == 'POST':
        ...
        while(flag):
            ...
            if act == 'unread':
                ...
                texttospeech("You will now be redirected to the menu page. Please click anywhere to start voice assistant on menu page.", file + i)
                i = i + str(1)
                conn.logout()
                return JsonResponse({'result': 'success'})
                ...
                texttospeech("Now you will be redirected back to the menu page. Please click anywhere to start voice assistant on menu page. ", file+i)
                i = i + str(1)
                return JsonResponse({'result': 'logout'})
